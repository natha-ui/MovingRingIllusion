...
parser.add_argument('--save', default=1000, type=int,
                    help='Period of save model and state (frames)')
parser.add_argument('--period', default=505000, type=int,
...
def train(device=torch.device("cpu")):
    ...
    scaler = torch.amp.GradScaler('cuda', enabled=args.useamp)
    count = 0
    seq = 0
    ...
    while count <= args.period:
        ...
        for data in tqdm(data_loader, unit="batch"):
            ...
            with torch.amp.autocast('cuda', enabled=args.useamp):
                data = data.to(device)
