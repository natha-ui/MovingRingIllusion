...
% The goal is continuous variation to prevent the network from stagnating.
%
...
% Dot direction: drawn uniformly each 8 frames (full 360)
% (direction is not normally distributed — fully random each block)

% Dot disappearance
DOT_DISAPPEAR_PROB = 0.10; % probability each dot disappears per 8-frame block

% ── Motion Cloud parameters ───────────────────────────────────────────────
MC_SF          = 1.0;
...
for seg = 1:N_SEGS
    ...
    for k = 1:FRAMES_PER_SEG
        ...
        if mod(k, 8) == 1
            ...
            if seg == 1
                % ── Dots: redraw size, speed, direction ────────────────
