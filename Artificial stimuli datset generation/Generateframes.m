% GenStimSequence400k.m
% Authors:  [Your Name] — based on CodeTestScriptD.m, DynTex.m, MotionCloud.m
%           by Andrew I Meso, Jonathan Vacher et al.
% Purpose:  Generate a 400,000-frame artificial stimulus sequence for DNN
%           neuroscience experiments. Stimuli known to drive V1 are used as
%           artificial analogues to natural motion sequences.
%
% Stimulus order  (100,000 frames each):
%   Segment 1  — Drifting sine-wave grating          (InNo 2)
%   Segment 2  — Moving dots                          (InNo 3)
%   Segment 3  — Square-wave grating                  (InNo 4)
%   Segment 4  — Motion Cloud (moving-ball envelope)  (MotionCloud)
%
% Transitions: cosine-ramp crossfade over the last 100 frames of segment N
%              and the first 100 frames of segment N+1.
%
% Saving strategy: frames are accumulated in a uint8 buffer and flushed to
%                  disk every SAVE_CHUNK frames (1,000 by default).  Each
%                  .mat file contains a variable "frames" of size
%                  [szDim x szDim x SAVE_CHUNK] uint8.
%
...
% ---- Helper: clamp and convert a double frame to uint8 ------------------
    function fr_u8 = toUint8(fr_dbl)
        fr_dbl = max(0, min(255, fr_dbl));
        fr_u8  = uint8(fr_dbl);
    end

% ---- Helper: generate a looped batch of N frames for a given generator --
%   generator_fn(frame_index_in_segment) → szDim x szDim double [0-255]
%   We pre-generate CHUNK_GEN frames at a time to reuse periodic stimuli.
...
%  Layout (frame indices, 1-based):
%
%  Seg1: 1 … 100000
%    └─ fade-OUT to Seg2: frames 99901 … 100000  (FADE_LEN=100)
%  Seg2: 100001 … 200000
%    └─ fade-IN  from Seg1: frames 100001 … 100100
%    └─ fade-OUT to Seg3:   frames 199901 … 200000
%  … and so on.
%
...
% =========================================================================
%% UTILITY: load and reconstruct full sequence from saved files
%  (run separately if needed — not executed during generation)
%
