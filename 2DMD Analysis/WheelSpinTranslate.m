% Script Name:  WheelSpinTranslate.m
...
% task. This version should translate left or right.
% Notes:        This version is created from a modification of the original
% script used for visualisation created by Nathan, but produced in vector
% form

function    WheelStimMat = WheelSpinTranslate(ImSiz, numSeg, radVec, edgePropSz, baseGry, nFrames, radSpd, dir)
...
% dir: movement direction either left or right for the motion
% direction. 1 or -1
...
for ii = 1:numSegDir
    % odd cases 
    if ii ~= (round(ii/2)*2)
        grayVals = [ grayVals baseGry(1) ]; % dark colour    
    end
    % even cases
    if ii == (round(ii/2)*2)
...
% B.5.1 for the directional movement, these are now fixed, limited to 8
% segments. NOTE TRANSLATION WILL ONLY WORK FOR 8 segments by default here
grayValsdir = [ baseGry(1) baseGry(1) baseGry(2) baseGry(1) ...
    baseGry(2) baseGry(2) baseGry(1) baseGry(2) ];
grayValsdirInverted = grayValsdir(end:-1:1);  % inverted for inner ring
...
for ii = 1:nFrames
    ...
    thetaInner = mod(theta_orig + phaseShift + pi/2, 2*pi); % inner ring rotated half segment more
    thetaOuter = mod(theta_orig + phaseShift + pi/4, 2*pi); % outer ring same as inner
    ...
    segmentIndexInner = floor(thetaInner / segWidthDir);
    for kk = 0:numSeg-1
        mask = (segmentIndexInner == kk) & innerRingMask;
        if dir == 1
            I1(mask) = grayValsdirInverted(kk+1);
        elseif dir == -1
            I1(mask) = grayValsdir(kk+1);
        end
    end

    % C.5 Outer ring segments, critical for the motion.. 
    segmentIndexOuter = floor(thetaOuter/segWidthDir);
    for kk = 0:numSeg-1
    ...
    % C.6 Output image matrix creation.. 
